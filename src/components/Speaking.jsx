  ...
  return (
    <div style={{ display: 'grid', gridTemplateRows: open ? '1fr' : '0fr', transition: 'grid-template-rows 0.38s cubic-bezier(0.22,1,0.36,1)' }}>
      <div style={{ overflow: 'hidden' }}>
        <div className="pt-4 pb-2 mt-4" style={{ borderTop: '1px solid rgba(245,242,238,0.08)' }}>
          {description && (
            <p className="font-body font-300 text-sm leading-relaxed mb-4" style={{ maxWidth: '60ch', color: 'rgba(245,242,238,0.45)' }}>
              {description}
            </p>
          )}
...
function FeaturedTalk({ talk, lang, t }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl mb-2" style={{ background: '#201C18', border: '1px solid rgba(245,242,238,0.07)' }}>
      {/* Warm glow */}
...
function TalkRow({ talk, lang, t }) {
  ...
  return (
    <div className="group py-4 last:border-b-0 -mx-4 px-4 transition-colors duration-150 rounded-sm hover:bg-white/[0.03]"
      style={{ borderBottom: '1px solid rgba(245,242,238,0.06)' }}>
      <div className="flex items-start gap-4 justify-between">
        <div className="flex-1 min-w-0">
          {/* Conference + intl badge */}
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="font-display font-400 transition-colors duration-200 group-hover:text-terra"
              style={{ fontSize: 'clamp(0.95rem, 1.8vw, 1.15rem)', letterSpacing: '-0.01em', color: 'rgba(245,242,238,0.82)' }}>
              {talk.conference}
            </span>
            {talk.international && (
          ...
          {/* Topic */}
          <p className="font-body font-300 leading-snug mb-2" style={{ fontSize: '0.85rem', color: 'rgba(245,242,238,0.45)' }}>
            {talk[`topic_${lang}`]}
          </p>
          {/* Speakers + location */}
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="font-body" style={{ fontSize: '0.75rem', color: 'rgba(245,242,238,0.28)' }}>
              {talk[`location_${lang}`]}
            </span>
          </div>
        </div>
...
export default function Speaking() {
  ...
  return (
    <section id="palestras" className="py-24 lg:py-36" ref={sectionRef}>
      <div className="max-w-editorial mx-auto px-6 lg:px-12">
        ...
        <div className="reveal mb-12">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <h2 className="font-display font-300 leading-tight"
              style={{ fontSize: 'clamp(1.9rem, 4vw, 3.6rem)', letterSpacing: '-0.02em', lineHeight: 1.05, color: 'rgba(245,242,238,0.88)' }}>
              {t('speaking.sectionTitle')}
            </h2>
            <p className="font-body font-300 leading-relaxed max-w-sm" style={{ fontSize: '0.9375rem', color: 'rgba(245,242,238,0.45)' }}>
        ...
        {/* Stats */}
        <div className="reveal reveal-delay-1 flex items-center gap-10 mb-10 pb-10" style={{ borderBottom: '1px solid rgba(245,242,238,0.08)' }}>
          {[
            ...
            <div key={label} className="flex items-baseline gap-2">
              ...
              <span className="font-body text-[0.7rem] tracking-[0.1em] uppercase" style={{ color: 'rgba(245,242,238,0.3)' }}>{label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="reveal reveal-delay-2 mt-2">
          {years.map((year) => (
            <div key={year} className="lg:grid lg:grid-cols-[72px_1fr] lg:gap-8 py-6 first:border-t-0"
              style={{ borderTop: '1px solid rgba(245,242,238,0.06)' }}>
              {/* Year anchor */}
              <div className="hidden lg:flex lg:items-start lg:pt-4">
                <span className="font-display font-300 sticky top-24"
                  style={{ fontSize: '0.9rem', letterSpacing: '0.06em', color: 'rgba(245,242,238,0.25)' }}>
                  {year}
                </span>
              </div>
              {/* Mobile year divider */}
